...
def init():
    ...
    if not user_whitelist:
        console.log('The user whitelist is empty, there will be no privileged users. Make sure the ".data/user_whitelist.txt" file exists and contains proper user IDs.', console.Level.WARNING)
    if not server_whitelist:
        console.log('The server whitelist is empty, experimental features won\'t be available to any server. Make sure the ".data/server_whitelist.txt" file exists and contains proper server IDs.', console.Level.WARNING)
